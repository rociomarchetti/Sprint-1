...
const getEmployee = valueID => {
    ...
    const employeeInfo = new Promise((resolve, reject) => {
        if (employees.find(employee => employee.id === valueID)) {
            ...
            const errorEmployee = 'No ha ingresado un id correcto'
            reject(console.error(errorEmployee))
        }  
    })
...
const getSalary = employeeObject => {
    ...
    const employeeSalary = new Promise((resolve, reject) => {
        if (employees.includes(employeeObject)) {
            ...
            const errorSalary = 'No ha ingresado los datos correctos de un empleado'
            reject(console.error(errorSalary))
        }
    })
...
getEmployee(4).catch(errorEmployee => console.log(errorEmployee))
getEmployee(-1).catch(errorEmployee => console.log(errorEmployee))
getEmployee(null).catch(errorEmployee => console.log(errorEmployee))
getEmployee(NaN).catch(errorEmployee => console.log(errorEmployee))
getSalary(4).catch(errorSalary => console.log(errorSalary))
getSalary(-1).catch(errorSalary => console.log(errorSalary))
getSalary(null).catch(errorSalary => console.log(errorSalary))
getSalary(NaN).catch(errorSalary => console.log(errorSalary))
